...
public class IPAddressTool {
    ...
    /*
     * @Description: 根据IP地址查询归属地（调用百度接口实现）
    ...
    public String getAddressById(String ip) {
        ...
        try {
            if (!Tool.isBlank(ip)) {
                ...
                log.info("开始调用百度接口[根据IP定位]");
                String resultStr = HttpUtil.get(url, CharsetUtil.charset(Constant.SYSTEM_CHARACTER_ENCODING));
                Map json = (Map) JSONObject.parse(resultStr);
                ...
                if ("0".equalsIgnoreCase(status)) {
                    ...
                    //获取到IP归属地
                    returnString = content.get("address").toString();
//                    System.out.println("address_detail=" + content.get("address_detail"));//获取结构化地址信息
//                    System.out.println("address=" + content.get("address"));//获取简要地址信息
//                    System.out.println("point=" + content.get("point"));//获取经度和纬度信息
                }
            }
        } catch (Exception e) {
