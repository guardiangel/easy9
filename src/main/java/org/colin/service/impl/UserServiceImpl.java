...
public class UserServiceImpl implements UserService {
    ...
    public List<DeptUserTree> getAllDeptUserTree() {
        ...
        if (null != userList && !userList.isEmpty()) {
            ...
            for (int i = 0; i < userList.size(); i++) {
                ...
                tree.setName(user.getRealName());
                tree.setChecked(nodeCheckedStatus);
                tree.setType(1);//类型（1：员工；2：部门；）
                tree.setPid(user.getDeptId());
                tree.setMonthSalary(user.getMonthSalary());//月薪
                result.add(tree);
            }
        }
        return result;
    ...
    public BbsUserVO getBbsUserInfoByPkId(String userId) {
        ...
        BbsUserVO vo = sysUserMapper.getBbsUserInfoByPkId(userId);
        vo.setPublishPostCount(publishPostCount);
    ...
    public void registerUser(RegisterUserReqVO vo, HttpServletRequest request) {
        ...
        String content = "注册邮箱为[" + vo.getEmail() + "]的[" + vo.getUsername() + "]用户注册[北方社区]成功，IP：" + ip + "，地点：[" + ip_address + "]";
        thirdTool.sendNoticeEmail("[北方社区]有人注册", content);
    }
    ...
    public SysUser getData1(UserAddReqVO vo){
    ...
    public SysUserAttac getData3(UserUpdateReqVO vo){
        SysUserAttac user = new SysUserAttac();
        user.setWagesNumber(vo.getWagesNumber());
        user.setSocialNumber(vo.getSocialNumber());
        user.setProvidentFundNumber(vo.getProvidentFundNumber());
